...
I would personally pick a list. I would think this because a set for example stores unique elements and doesn't allow duplicates which is what we would not want in this case. 
And why a list instead of a bag? A bag allows duplicates but it remains unordered. A use item based menu would most likely require a predictable sequence so that the player can find 
the item they'd like to use with ease.
...
I would say linear because it is sorted by a number and not name, if it was sorted by name it would be better to use binary search.
The big O for linear would be O(n) and for binary it would be O(log n).
...
auto comp = [](const Item& a, const Item& b) { return a.value < b.value; }; 
...
So Bag<T> lives in Bag.h since it is a template class because it needs to be seen to work with any type T. If it was in Bag.cpp the "definition" would not be seen and would cause compiler errors.
